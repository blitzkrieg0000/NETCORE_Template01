...
public class CustomAuthService : ICustomAuthService {
    ...
    #region Deneme
    public async Task<Response<TokenDto>> GoogleSignInAsync() {
        var result = await _httpContextAccessor.HttpContext.AuthenticateAsync(OpenIdConnectDefaults.AuthenticationScheme);
        var id_token = await _httpContextAccessor.HttpContext.GetTokenAsync(OpenIdConnectDefaults.AuthenticationScheme, "id_token");
        /*
        ...
        */
        ...
        if (result.Succeeded) {
            TokenDto token = new();
            token.AccessToken = await _httpContextAccessor.HttpContext.GetTokenAsync(GoogleDefaults.AuthenticationScheme, "access_token");
            ...
            return new(ResponseType.Success, token, "Google ile giriş başarılı.");
        }
        return new(ResponseType.ValidationError, "Giriş başarısız.");
    }
    #endregion
    #endregion
