...
public class AuthController : Controller {
    ...
    #region Google Login Denemesi
    // [AllowAnonymous]
    // public async Task GoogleLogin() {
    //     // https://localhost:7094/Auth/GoogleLogin
    //     await HttpContext.ChallengeAsync(OpenIdConnectDefaults.AuthenticationScheme, new AuthenticationProperties() {
    //         RedirectUri = Url.Action("GoogleLoginAuth", "Auth")
    //     });
    // }
    ...
    // [AllowAnonymous]
    // // [FromForm]TempTokenModel value
    // public async Task<IActionResult> GoogleLoginAuth() {
    //     var method = HttpContext.Request.Method;
    //     System.Console.WriteLine(method);
    //     var result = await HttpContext.AuthenticateAsync(OpenIdConnectDefaults.AuthenticationScheme);
    //     var id = await HttpContext.GetTokenAsync(OpenIdConnectDefaults.AuthenticationScheme, "id_token");

    //     // var response = await _mediator.Send(new GoogleSignInRequest());
    //     // return this.ResponseRedirectToAction(response, "Index", "Admin");
    //     return Json(result);
